...
import design.sandwwraith.mlhw.Deducer
...
object Deductions {
  ...
  private def lemma1InferFA(a: Expr, b: Expr, c: Expr): List[Expr] = { //(A&B->FA(x,C)) -> (A->B->FA(x,C))
    new Deducer().apply(List((a&b) ->: c, a, b), Some(c), List(a, b, a ->: (b->:(a&b)), b ->: (a&b), a & b, (a&b) ->: c, c)) match {
      case Right(proof1) =>
        new Deducer().apply(List((a&b) ->: c, a), Some(b ->: c), proof1._3) match {
          case Right(proof2) =>
            new Deducer().apply(List((a&b) ->: c), Some(a ->: (b ->: c)), proof2._3) match {
  ...
  private def lemma2InferFA(a: Expr, b: Expr, c: Expr): List[Expr] = { //(A->B->C) -> (A&B->C)
    new Deducer().apply(List(a ->: b ->: c, a&b), Some(c), List((a&b) ->: a, a&b, a, (a&b) ->: b, b, a ->: (b ->: c), b ->: c, c)) match {
        case Right(proof1) =>
          new Deducer().apply(List(a ->: b ->: c), Some(a&b ->: c), proof1._3) match {
            case Right(finalProof) => finalProof._3.toList
  ...
  def lemmaEX(a: Expr, b: Expr, c: Expr): List[Expr] = { //(A->B->C) -> (B->A->C)
    new Deducer().apply(List(a ->: (b ->: c), b, a), Some(c), List(a ->: (b ->: c), a, b ->: c, b, c)) match {
      case Right(proof1) =>
        new Deducer().apply(List(a ->: (b ->: c), b), Some(a ->: c), proof1._3) match {
          case Right(proof2) =>
            new Deducer().apply(List(a ->: (b ->: c)), Some(b ->: (a ->: c)), proof2._3) match {
              case Right(finalProof) => finalProof._3.toList
